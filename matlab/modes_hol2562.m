...
if readdata==1
    ...
    e1=eigs(A, 2560);   %%sizeofmatriz -2 to be able to use sparse matrix - matlab limitation
     ...
     e10=eigs(A, 2560);   %%sizeofmatriz -2 to be able to use sparse matrix - matlab limitation
      ...
      B=dlmread('swm_tc32_HTC_vrectrsk_crectrsk_sinttrsk_grdtrsk_hol100.0_linearmatrixsparse_HR95JT_003.txt');
     A=sparse(B(:,1),B(:,2), B(:,3));
     e100=eigs(A, 2560);   %%sizeofmatriz -2 to be able to use sparse matrix - matlab limitation
...
e10_i=sort(imag(e10));
e10_r=sort(real(e10));
e100_i=sort(imag(e100));
e100_r=sort(real(e100));
...
plot(e100_r ,[linestyles{3} Markers(1)],'LineWidth',linewidth, ...
    'MarkerSize', marksize, 'DisplayName', char(names1(3)), 'Color', colors(3,:))
xlabel('','fontsize',12)
ylabel('Frequency','fontsize',12)
title('Modes Real','fontsize',12 )
box off;
...
plot(e1_i ,[linestyles{2} Markers(1)],'LineWidth',linewidth, ...
    'MarkerSize', marksize, 'DisplayName', char(names1(4)), 'Color', colors(1,:))
hold on
plot(e10_i ,[linestyles{3} Markers(1)],'LineWidth',linewidth, ...
    'MarkerSize', marksize, 'DisplayName', char(names1(5)), 'Color', colors(2,:))
plot(e100_i ,[linestyles{4} Markers(1)],'LineWidth',linewidth, ...
    'MarkerSize', marksize, 'DisplayName', char(names1(6)), 'Color', colors(3,:))
xlabel('Modes','fontsize',12)
ylabel('Frequency','fontsize',12)
title('Modes Imag','fontsize',12 )
box off;
...
plot(e1 , 'o')
hold on
plot(e10 , 'd', 'Color', 'Red' )
plot(e100 , 'x', 'Color', 'Green' )
xlabel('Real','fontsize',12)
ylabel('Imag','fontsize',12)
title('Modes','fontsize',12 )
legend('h1', 'h10', 'h100')
box off;
hold off
