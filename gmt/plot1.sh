...
if [ $vec ] ; then
    ...
    #awk '{ if ( NR % 8 == 0 ) { print $1, $2, $3, $4/200 ;} }' $vec.dat > 'vec.tmp'
    ...
    #awk '{ { print $1 , $2, $3, $4 ;} }' $vec.dat > 'vec.tmp'
    #awk '{ { print $1, $2, $3, $4 ;} }' $vec.dat > 'vec.tmp'

    #Draw vectors
    # arrowwidth/headlength/headwidth  
    vecstyle="0.01/0.04/0.06"
...
#echo "-143.703296703 -89.9010989010 " > point1.dat
#echo "-143.604395604 -89.9010989010 " > point2.dat
#echo "-143.604395604 -89.8021978022 " > point3.dat
#echo "-143.703296703 -89.8021978022 " > point4.dat
echo "-180 -90" > 'point.dat'
#echo "-23.8744234108428        34.7927455551923" > 'point.dat'
psxy  point.dat  $map -Sx0.0005c  -W0.0005/black -Gred -O -K >> $plot
...
#psxy point4.dat  $map -Sx0.1c  -W0.5/red -Gred -O  -V >> $plot
