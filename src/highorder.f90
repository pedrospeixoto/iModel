...
contains   
  ...
  subroutine highordertests(mesh)
    ...
    do i = 1,nodes
       ...
       ngbr = mesh%v(i)%nnb
       if((advmtd=='og3a' .or. advmtd=='og3b' .or. advmtd=='og3c') .and. ngbr<5)then
    ...
    if (order > 2 .and. advmtd/='og3a' .and. advmtd/='og3b' .and. advmtd/='og3c') then
...
subroutine compute_error_zonal_wind(mesh, time, step)
  ...
  subroutine allocation(nodes,mesh)
    ...
    do i=1,nodes
       ...
       ngbr2=0
       if(order>2 .and. advmtd/='og3a' .and. advmtd/='og3b' .and. advmtd/='og3c')ngbr2=node(i)%ngbr(2)%numberngbr
  ...
  subroutine stencil(nodes,mesh)
    ...
    if(order==2 .or. advmtd=='og3a' .or. advmtd=='og3b' .or. advmtd=='og3c')then
       do i=1,nodes
          x=mesh%v(i)%p(1)
          y=mesh%v(i)%p(2)
          z=mesh%v(i)%p(3) 
...
subroutine reconstruction_gas(nodes)
  ...
  subroutine coordsbarmid(nodes,mesh)  
    ...
    do i=1,nodes
       ...
       call aplyr(x,y,z,cx,sx,cy,sy,xp,yp,zp)
       jend=node(i)%ngbr(1)%numberngbr
       if(order>2 .and. advmtd/='og3a' .and. advmtd/='og3b' .and. advmtd/='og3c')jend=jend+node(i)%ngbr(2)%numberngbr
  ...
  subroutine gaussedges(nodes)
    ...
    if(method=='G' .or. advmtd=='og3c') diml=1
    allocate(x(diml),w(diml))
    call gaussrootsweights(diml, x, w)
    do i=1,nodes
       jend=node(i)%ngbr(1)%numberngbr
...
subroutine flux_olg(nodes,mesh,z,time)
    ...
    !$OMP SHARED(order, controlvolume, feps, flux_olg_rot, advmtd) &
  ...
  subroutine init_quadrature_edges(mesh)  
    ...
    if (method=='G' .or. advmtd=='og3c') then
      nquad = 1
    end if
  ...
  subroutine monotonicfilter_rk3_v2(mesh, phi_step0, phi_step2, dt, radius, time, u_step0, u_step2, hSphi)
    ...
    type(grid_structure), intent(inout) :: mesh
    type(scalar_field), intent(inout) :: phi_step0
    type(scalar_field), intent(inout) :: phi_step2
    type(scalar_field), optional, intent(inout) :: u_step0
    type(scalar_field), optional, intent(inout) :: u_step2
  ...
  subroutine divhx(q, div, mesh, radius, time)
   ...
   .or. advmtd=='og2' .or. advmtd=='og3' .or. advmtd=='og3a' .or. advmtd=='og3b' .or. advmtd=='og3c' .or. advmtd=='og4') then
      !$omp parallel do &
      !$omp default(none) &
      ...
      if (advmtd=='og2' .or. advmtd=='og3' .or. advmtd=='og3a' .or. advmtd=='og3b' .or. &
          advmtd=='og3c' .or. advmtd=='og4')then ! Ollivier-Gooch method
        !---------------------------------------------------------
