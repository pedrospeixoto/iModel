...
contains
  ...
  !
  !! STRIPACK 
  !   FORTRAN90 library which carries out some computational 
  !   geometry tasks on the unit sphere in 3D
  !
  !
  !  Modified:
  !
  ...
  !
  !
  !  OBS: The STRIPACK was converted to a module STRIPACKM by 
  !		Pedro Peixoto (pedrosp@ime.usp.br) on 17 August 2010
  !
  !
  !************************************************************************
...
1   continue
  ...
  function jrand ( n, ix, iy, iz )
    ...
    u = x - int ( x )
    jrand = int( real ( n, kind = 8 ) * u + 1.0D+00, 4)
  ...
  function nrand ( n, n0 )
    ...
    !*****************************************************************************80
    !
    !! NRAND returns a random integer between 1 and N.
    !
    !  Discussion:
    !
    !   This function returns a uniformly distributed pseudorandom integer
    ...
    !    Input, integer ( kind = 4 ) N, the maximum value to be returned.
    !
    !    Input, integer ( kind = 4 ) N0, a seed
    !
    !    Output, integer ( kind = 4 ) NRAND, a random integer in the range 1 to N.
    !
    implicit none
    ...
    integer (kind = 4):: rand_seed
    integer (kind = 4):: the_size
    integer (kind = 4):: j
    ...
    !Generates pseudo-random number in [0,1]
    call random_number(r)

    randn = real ( n, kind = 8 ) * r + 1.0D+00
    nrand = int( randn, 4)
...
4         continue
  ...
  subroutine trans ( n, rlat, rlon, x, y, z )
    ...
    do i = 1, nn
       ...
       theta = rlon(i)
       cosphi = dcos ( phi )
       x(i) = cosphi * dcos ( theta )
       y(i) = cosphi * dsin ( theta )
       z(i) = dsin ( phi )
    end do
  ...
  subroutine trfind ( nst, p, n, x, y, z, list, lptr, lend, b1, b2, b3, i1, &
    ...
    if ( n0 < 1 .or. n < n0 ) then
       ...
       print*, "TRFIND Warning: Random in the beginning"
       print*, "   Node index given is outside mesh range", n0, n
       print*, "   Using random index in 1,2,...,n"
    end if
    !
...
8   continue
    ...
    if ( b3 >= eps ) then
       ...
       if ( b1 < -tol .or. b2 < -tol ) then
          !n0 = jrand ( n, ix, iy, iz )
          n0 = nrand ( n, n0 ) !P. Peixoto
          print*, "Random in barycenter coords:", n0, n
          go to 2
       end if
...
106 format (i5,3e15.6,4x,5i5/(1x,53x,5i5))
  ...
  subroutine vrplot ( lun, pltsiz, elat, elon, a, n, x, y, z, nt, listc, lptr, &
    ...
    if ( numbr ) then
       ...
       do n0 = 1, n
          ...
          if ( x0 * x0 + y0 * y0 <= wrs ) then
             write ( lun, '(2f14.6,a)' ) x0, y0, ' moveto'
             write ( lun, '(a,i3,a)' ) '(', n0, ') show'
          end if
    ...
    y0 = wr + 3.0D+00 * t
    write ( lun, '(a)' ) title
    write ( lun, '(a,g14.6,a)' ) '  stringwidth pop 2 div neg ', y0, ' moveto'
    write ( lun, '(a)' ) title
    write ( lun, '(a)' ) '  show'
    !
    !  Display the window center and radius below the plot.
