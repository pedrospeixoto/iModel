...
SUBROUTINE setconst
  ...
  CASE(8)
     phiref = 0.05d0*gravity ! for Hollingsworth test
...
SUBROUTINE integrate
  ...
  REAL*8 :: phiforce(nx,ny), uforce(nx,ny), vforce(nx,ny+1), upert(nx,ny)
  integer::i, j
  ...
  DO istep = 1, nstop
     ...
     IF (MOD(istep,idump) == 0 .and. idumpref) THEN
        !CALL outstate(istep)
	 ...
	 IF(ic==8 .and. .true.)THEN !Hollingsworth test with linearizations
	 	IF(istep==1)THEN
	 		...
	 		!Perturb phi
	 		!phi(nx/2, ny/2)  =phi(nx/2, ny/2)  +phi(nx/2, ny/2)  /100.0
	 		!phi(nx/2+1, ny/2)=phi(nx/2+1, ny/2)+phi(nx/2+1, ny/2)/100.0
	 		!phi(nx/2, ny/2+1)=phi(nx/2, ny/2+1)+phi(nx/2, ny/2+1)/100.0
	 	END IF
		...
		if(u_maxerr>0) nonlin_alpha=0.001/u_maxerr
...
SUBROUTINE writeref
  ...
  DO igref=1,ngref
     ...
     yrefpre( igref) = 'TC'//trim(icname)//'_'//trim(ygc(igref))
  END DO
...
SUBROUTINE plot_var(q, ytitle, pos)
  ...
  CHARACTER (LEN=256):: filename, icname
  ...
  WRITE(icname,'(I4.2)') ic
  icname=trim(adjustl(trim(icname)))
  filename="dump/"//"eg_swe_ic"//trim(icname)//"_"//trim(ytitle)//".dat"
