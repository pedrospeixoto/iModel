...
!INTEGER, PARAMETER :: p = 9, nx = 2**p, ny = 2**(p-1)
...
elseif (ic == 11 )then
  ...
  DO j = 1, ny
    DO i = 1, nx
	   ...
	   den = (yu(j))**2 + ( -(2d0*pi-xu(i)) )**2
	   end if
       u0(i,j) = 50.0*exp(-den*10.0)
    ENDDO
    !print '(129f10.5)', yu(j), xu(1:nx)

  ENDDO

  DO j = 1, ny+1
    DO i = 1, nx
      v0(i,j) = 0d0 !u00*COS(xv(i))*sinr
    ENDDO
  ENDDO

call dumpm(phi0,'phi0',nx,ny)
call dumpm(u0,'u0',nx,ny)
...
! List of reference solution file prefixes
   yrefpre( igref) = 'TC42_'//trim(ygc(igref))
end do
...
if(istep==1)then
	OPEN(58,FILE='TC42_reftimes.dat', status='replace')
	write(58, *) nreftime
	DO ilist = 1, nreftime
